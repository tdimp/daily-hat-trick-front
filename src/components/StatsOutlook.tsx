...
const StatsOutlook = ({ player }: PlayerPropsInterface) => {
  ...
  useEffect(() => {
    if (player.nhl_team) {
      fetch(`https://statsapi.web.nhl.com/api/v1/schedule?teamId=${player.nhl_team.id}&startDate=${startDate}&endDate=${endDate}`)
      .then(res => res.json())
    ...
    if (currentDay === 0) {
      ...
      endDate = `${currentDate.getFullYear()}-${('0' + (currentDate.getMonth() + 1)).toString().slice(-2)}-${'0' + currentDate.getDate().toString().slice(-2)}`
